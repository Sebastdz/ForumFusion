...
<html>
  ...
  <button class="menu-dropdown-button" onclick="toggleDropdownMenu2()"><i class="fa-solid fa-bars"> </i></button>
  <div class="menu-dropdown-content" id="mainmenudropdown2">
    <button class="logged-postcomment" onclick="newpost2()"><i class="fa-solid fa-plus"></i> New Comment</button>
    <input type="text" id="searchbar2" placeholder="Search Comments..." onkeydown="searchOnEnterComments(event)">
    <!-- <button onclick="performAction('Option 2')">Option 2</button>
    <button onclick="performAction('Option 3')">Option 3</button> -->
    <!-- Add more buttons as needed -->

    <script>
function performSearch2(postId) {
    var searchValue = document.getElementById("searchbar2").value;

    // Encode the search term to handle special characters
    var encodedSearchValue = encodeURIComponent(searchValue);

    // Redirect to the page with the search term and post ID as query parameters
    window.location.href = 'full_post.php?post_id=' + postId + '&searchComment=' + encodedSearchValue;
}


function searchOnEnterComments(event) {
    if (event.key === "Enter") {
        performSearch2();
    }
}
...
function toggleDropdownMenu2() {
  var dropdownMenu2 = document.getElementById("mainmenudropdown2");
  dropdownMenu2.classList.toggle("show");
}

// Close the dropdown if the user clicks outside of it
window.onclick = function(event2) {
  var isMenuButton2 = event2.target.matches('.menu-dropdown-button') || event2.target.closest('.menu-dropdown-button');
  
  if (!isMenuButton2) {
    var dropdownMenus2 = document.getElementsByClassName("menu-dropdown-content");
    for (var i = 0; i < dropdownMenus2.length; i++) {
      var openDropdownMenu2 = dropdownMenus2[i];
      if ((openDropdownMenu2.classList.contains('show') && !event2.target.matches('.logged-postcomment')) && !event.target.matches('#searchbar2')) {
        openDropdownMenu2.classList.remove('show');
      }
    }
  }
}
...
?>
  ...
  <?php 
    ...
    if($rank=="Banned")
    {
      ...
      echo "<p class='post-user'>" . "[banned] <span class='$rankClass'>$rank</span>";
    }
    else 
    {
...
<?php 
     if (isset($_SESSION['admin_logged_in']) && $_SESSION['admin_logged_in']) {
      echo "<button class='deletebutton' onclick='deletePost(" . $postData['post_id'] . ")'><i class='fa-solid fa-trash'></i></button>";
      echo "<button class='deletebutton' onclick='deletePostByStaff(" . $postData['post_id'] . ")'><i class='fa-solid fa-delete-left'></i></button>";
      echo "<button class='deletebutton' onclick='banUser(\"" . $postData['post_id'] . "\")'><i class='fa-solid fa-user-slash'></i></button>";
  } 
    ...
    // Check if the user is a premium user and is the owner of the post
        ...
        echo "<button class='deletebutton' onclick='deletePostByUser(" . $postData['post_id'] . ")'><i class='fa-solid fa-trash'></i></button>";
    } 
...
if (isset($_GET['searchComment']) && !empty($_GET['searchComment'])) {
    $searchCommentPhrase = mysqli_real_escape_string($conn, $_GET['searchComment']);

    // Modify the SQL query to search for comments with the provided search term
    $commentsQuery = "SELECT * FROM comments WHERE post_id = ? AND comment_content LIKE '%$searchCommentPhrase%'";
} else {
    // No search term provided, retrieve all comments
    ...
    while ($commentData = mysqli_fetch_assoc($commentsResult)) {
        ...
        $commentDate = $commentData['comment_date'];
            ...
            if($crank=="Banned")
            {
              ...
              echo "<p class='post-user'>" . "[banned] <span class='$crankClass'>$crank</span>";
            }
            else 
            {
            ...
            echo "</div>";
            if (isset($_SESSION['admin_logged_in']) && $_SESSION['admin_logged_in']) {
              echo "<button class='deletebuttoncomment' onclick='deleteComment(" . $commentId . ")'><i class='fa-solid fa-trash'></i></button>";
              echo "<button class='deletebuttoncomment' onclick='deleteCommentByStaff(" . $commentId . ")'><i class='fa-solid fa-delete-left'></i></button>";
              echo "<button class='deletebuttoncomment' onclick='banUserComment(" . $commentId . ")'><i class='fa-solid fa-user-slash'></i></button>";
            }
            else if (isset($_SESSION['moderator_logged_in']) && $_SESSION['moderator_logged_in']) {
              echo "<button class='deletebuttoncomment' onclick='deleteComment(" . $commentId . ")'><i class='fa-solid fa-trash'></i></button>";
              echo "<button class='deletebuttoncomment' onclick='deleteCommentByStaff(" . $commentId . ")'><i class='fa-solid fa-delete-left'></i></button>";
          ...
          echo "<button class='deletebuttoncomment' onclick='deleteCommentByUser(" . $commentData['comment_id'] . ")'><i class='fa-solid fa-trash'></i></button>";
      } 
      // Check if the user is a regular user and is the owner of the post
...
<form method="post" action="process_comment.php">
    ...
    <input type="text" name="comment_content" placeholder="Your comment" required>
...
function sticky_header() {
  ...
  var newPostButton2 = document.getElementsByClassName('menu-dropdown')[0];
...
function deleteComment(commentId) {
    ...
    function banUserComment(commentId) {
        if (confirm("Are you sure you want to ban this user?")) {
            // If the user confirms the deletion, send an AJAX request to the server
            var xhr = new XMLHttpRequest();
            xhr.onreadystatechange = function() {
            ...
            xhr.open("POST", "banUserComment.php", true);
            xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            xhr.send("comment_id=" + commentId);
        }
    }
