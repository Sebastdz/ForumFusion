...
<html>
  ...
  <button class="logged-postcomment" onclick="newpost()"><i class="fa-solid fa-plus"></i> <span class="button-text">New Post</span></button>
...
<span class="close2" onclick="xpostcomment()">&times;</span>
        ...
        <input type="text" name="post_content" placeholder="Your post">
...
function newpost() {
  ...
  var newPostButton = document.getElementsByClassName('logged-postcomment')[0];

  // Check if the session variable exists before showing the button
   if (<?php echo isset($_SESSION['user_logged_in']) ? 'true' : 'false'; ?>) {
    newPostButton.style.display = 'block'; // Show the button
    newPostButton.onclick = newpost;
  } else {
    newPostButton.style.display = 'none'; // Hide the button
  }
